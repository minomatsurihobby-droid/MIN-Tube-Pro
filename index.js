...
<head>
    ...
    <style>
        :root { 
            ...
            --ai-glow: 0 0 15px rgba(155, 114, 203, 0.3);
        }
        ...
        .ai-close-btn { color: var(--text-sub); cursor: pointer; font-size: 18px; background: none; border: none; padding: 4px; transition: 0.2s; }
        .ai-close-btn:hover { color: white; transform: scale(1.1); }
        
        /* Gemini-style Loading */
        .ai-loading-wrapper { padding: 20px 0; text-align: center; }
        .ai-loading-bar { 
            height: 4px; width: 100%; background: #1a1a1a; position: relative; overflow: hidden; border-radius: 10px; margin-bottom: 12px;
        }
        .ai-loading-progress { 
            position: absolute; width: 40%; height: 100%; 
            background: var(--ai-gradient); 
            animation: ai-slide 1.8s infinite ease-in-out; 
            filter: blur(1px);
        }
        @keyframes ai-slide { from { left: -50%; } to { left: 150%; } }
        ...
        #aiJumpBtn:hover { transform: scale(1.05); border-color: #9b72cb; box-shadow: var(--ai-glow); }
        #aiJumpBtn i { background: var(--ai-gradient); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }

        /* Video Elements */
        .player-container { width: 100%; aspect-ratio: 16 / 9; background: black; border-radius: 12px; overflow: hidden; position: relative; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
        .video-title { font-size: 20px; font-weight: bold; margin: 16px 0 8px 0; line-height: 28px; }
        ...
        .description-box { background: var(--bg-secondary); border-radius: 12px; padding: 12px; font-size: 14px; margin-bottom: 24px; white-space: pre-wrap; }
        
        /* Recommendation Items */
        .rec-item { display: flex; gap: 12px; margin-bottom: 12px; cursor: pointer; text-decoration: none; color: inherit; transition: 0.2s; }
        .rec-item:hover { background: rgba(255,255,255,0.05); border-radius: 8px; }
        ...
        .video-loading-overlay { position: absolute; inset: 0; background: rgba(0,0,0,0.8); display: none; flex-direction: column; align-items: center; justify-content: center; z-index: 10; gap: 15px; }
        .video-loading-overlay.active { display: flex; }
        .spinner { width: 40px; height: 40px; border: 4px solid rgba(255,255,255,0.1); border-top-color: var(--yt-red); border-radius: 50%; animation: spin 1s linear infinite; }
        ...
        @media (max-width: 1000px) { 
            ...
            #aiJumpBtn { top: 60px; right: 10px; padding: 8px 12px; }
        }
    </style>
</head>
<body>
...
<nav class="navbar">
    ...
    <div class="nav-center">
        <form class="search-bar" action="/nothing/search"><input type="text" name="q" placeholder="検索"><button type="submit" class="search-btn"><i class="fas fa-search"></i></button></form>
    </div>
    <div style="width:100px;"></div>
</nav>

<button id="aiJumpBtn" onclick="triggerAIRecommendationUI()">
    <i class="fas fa-sparkles"></i> AIのおすすめを表示
</button>
...
<div class="container">
    <div class="main-content">
        ...
        <div class="comments-section">
            ...
            ${commentsData.comments.map(c => \`<div style="display:flex; gap:16px; margin-bottom:20px;">
                <img style="width:40px; height:40px; border-radius:50%;" src="\${c.authorThumbnails?.[0]?.url || ''}">
                <div>
                    <span style="font-weight:bold; font-size:13px; margin-bottom:4px; display:block;">\${c.author}</span>
    ...
    <div class="sidebar">
        <div id="aiRecContainer" class="ai-rec-container">
            <div class="ai-rec-header">
                <div class="ai-badge"><i class="fas fa-sparkles"></i> AIのおすすめ動画</div>
                <button class="ai-close-btn" onclick="dismissAI()"><i class="fas fa-times"></i></button>
            </div>
            <div id="aiRecContent">
                <div class="ai-loading-wrapper">
                    <div class="ai-loading-bar"><div class="ai-loading-progress"></div></div>
                    <div style="font-size:12px; color:var(--text-sub);">履歴を解析して次に見るべき動画を選んでいます...</div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function saveHistory() {
        let history = JSON.parse(localStorage.getItem('yt_pro_history_v2') || '[]');
        const isExists = history.some(h => h.id === CURRENT_VIDEO_ID);
        if (!isExists) {
            history.unshift({ id: CURRENT_VIDEO_ID, title: CURRENT_TITLE, channel: CURRENT_CHANNEL, timestamp: Date.now() });
        }
        localStorage.setItem('yt_pro_history_v2', JSON.stringify(history.slice(0, 50)));
    }
    ...
    async function getAIRecommendation() {
        const history = JSON.parse(localStorage.getItem('yt_pro_history_v2') || '[]');
        const subs = getSubs();
        
        if (history.length < 1) return null;

        const historyPrompt = history.slice(0, 15).map(h => \`- \${h.title} (Channel: \${h.channel})\`).join('\\n');
        const subsPrompt = subs.length > 0 ? subs.join(', ') : 'なし';

        const prompt = \`
あなたはYouTubeの高度な推薦アルゴリズムです。以下のユーザー情報を基に、このユーザーが次に見たいと思うYouTube動画のタイトルを1つだけ推測して提案してください。
...
指示：
1. 今までの傾向から、興味関心が最も高いジャンルの動画を選んでください。
2. 出力は必ず「動画のタイトル」という形式で、タイトルを「」で囲んで出力してください。余計な説明は一切不要です。
\`;
        ...
        try {
            ...
            const targetTitle = match ? match[1] : text.replace(/[「」]/g, "").trim();
            
            return await searchVideoOnBackend(targetTitle);
        } catch (e) {
    ...
    async function searchVideoOnBackend(query) {
        try {
            const params = new URLSearchParams({ title: query, channel: "", id: "search_mode" });
            const res = await fetch(\`/api/recommendations?\${params.toString()}\`);
            const data = await res.json();
            // 検索結果の最初の一つを「正しいデータ」として採用
    ...
    async function initAI() {
        if (localStorage.getItem('ai_rec_disabled_v2') === 'true') return;

        const isShownOnce = localStorage.getItem('ai_rec_initialized') === 'true';
        const aiContainer = document.getElementById('aiRecContainer');
        const jumpBtn = document.getElementById('aiJumpBtn');

        if (!isShownOnce) {
            // 初回表示
            aiContainer.style.display = 'block';
            localStorage.setItem('ai_rec_initialized', 'true');
            loadAndRenderAI();
        } else {
            // 2回目以降はジャンプボタンを表示
    ...
    async function loadAndRenderAI() {
        const content = document.getElementById('aiRecContent');
        const video = await getAIRecommendation();

        if (!video) {
            content.innerHTML = '<div style="font-size:12px; color:var(--text-sub); text-align:center; padding:10px;">おすすめを見つけられませんでした。もっと動画を見て履歴を増やしてください。</div>';
            return;
        }

        content.innerHTML = \`
            <a href="/video/\${video.id}" class="rec-item" style="margin-bottom:0; padding:8px;">
                <div class="rec-thumb">
                    <img src="https://i.ytimg.com/vi/\${video.id}/mqdefault.jpg">
                    <div style="position:absolute; bottom:4px; right:4px; background:rgba(0,0,0,0.8); font-size:10px; padding:2px 4px; border-radius:2px; font-weight:bold; color:#9b72cb;">AI PICK</div>
                </div>
                <div class="rec-info">
                    <div class="rec-title" style="font-size:14px;">\${video.title}</div>
                    <div class="rec-meta" style="margin-top:4px;">\${video.channelTitle}</div>
                    <div class="rec-meta" style="color:#9b72cb; font-weight:bold; margin-top:2px;"><i class="fas fa-sparkles"></i> あなたへの最適解</div>
                </div>
            </a>
        \`;
    }
    ...
    function dismissAI() {
        if (confirm("AIおすすめ機能を非表示にしますか？")) {
            localStorage.setItem('ai_rec_disabled_v2', 'true');
            document.getElementById('aiRecContainer').style.display = 'none';
            document.getElementById('aiJumpBtn').style.display = 'none';
        }
    }
    ...
    async function loadRecommendations() {
        const params = new URLSearchParams({ title: CURRENT_TITLE, channel: CURRENT_CHANNEL, id: CURRENT_VIDEO_ID });
        const res = await fetch(\`/api/recommendations?\${params.toString()}\`);
        const data = await res.json();
